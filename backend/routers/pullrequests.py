...
async def generate(session_id: str, db: AsyncSession = Depends(get_db)):
    ...
    for i, pr in enumerate(raw):
        raw_id = pr.get("id") or f"PPR-{str(i+1).zfill(3)}"
        obj = PolicyPullRequest(
            ...
            implementation_steps=pr.get("implementation_steps") or [],
            estimated_effort=pr.get("estimated_effort") or "1-2 weeks",
            status="pending",
        )
        db.add(obj)
        prs.append(obj)
...
def _gap_dict(g): return {"id": g.id, "obligation_id": g.obligation_id, "coverage": g.coverage, "risk_level": g.risk_level, "explanation": g.explanation, "cited_source": g.cited_source, "recommended_action": g.recommended_action or ""}
def _serialize(p): return {
    ...
    "estimated_effort": p.estimated_effort or "1-2 weeks",
    "status": p.status,
}
