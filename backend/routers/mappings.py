...
async def map(session_id: str, db: AsyncSession = Depends(get_db)):
    ...
    # Sort by confidence desc so we always keep the highest-confidence mapping when deduplicating
    raw_sorted = sorted(raw, key=lambda x: float(x.get("confidence", 0)), reverse=True)
    for i, m in enumerate(raw_sorted):
        ob_id = m.get("obligation_id", "")
        # Enforce single best-match per obligation (reviewer requirement)
