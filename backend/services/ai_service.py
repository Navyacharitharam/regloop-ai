...
async def _call_async(prompt: str, max_tokens: int = 4096, retries: int = 3) -> str:
    ...
    for attempt in range(retries):
        try:
            ...
            last_exc = exc
            msg = str(exc).lower()
            # Never retry bad-key / permission errors — surface them immediately
            if any(k in msg for k in ("api_key", "invalid_argument", "permission", "unauthenticated")):
                raise
...
- Use the owner from the responsibility matrix where the domain matches"""
    try:
        return _parse(await _call_async(prompt, max_tokens=6000))
    except Exception:
