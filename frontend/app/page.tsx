...
interface SessionItem {
  ...
  policy_names: string[];
  created_at: string;
  _status?: { obligations: number; gaps_done: number; not_covered: number; prs: number; reviews: number };
}
    ...
    Promise.all(
      ids.map(id =>
        api.getSession(id)
          .then(async sess => {
            try {
              const st = await api.getSessionStatus(id);
              return { ...sess, _status: { obligations: st.obligations, gaps_done: st.gaps, not_covered: 0, prs: st.pull_requests, reviews: 0 } };
            } catch {
              return sess;
            }
          })
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <div className="text-xs mt-0.5" style={{ color: "var(--silver)" }}>
                        ...
                        {s._status && s._status.obligations > 0 && (
                          <span className="ml-2 px-1.5 py-0.5 rounded font-semibold" style={{ background: "rgba(0,180,216,0.1)", color: "var(--teal)" }}>
                            {s._status.obligations} obligations · {s._status.prs} PRs
                          </span>
                        )}
                      </div>
                    </div>
