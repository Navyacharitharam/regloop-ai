...
export default function WorkspacePage() {
  ...
  const loadExisting = useCallback(async () => {
    try {
      ...
      if (prRes.pull_requests.length > 0) {
        ...
        if (audit) { setAuditTrail(audit.trail); setAuditEvents(audit.raw_events); setSessionName(audit.regulation); }
      }
    } catch { /* new session */ }
  }, [sid]);
  ...
  async function runFullPipeline() {
    ...
    setStepMsg("Extracting obligations from regulation...");
    try {
      const r1 = await api.extractObligations(sid);
    ...
    setStepMsg("Mapping obligations to policy sections...");
    try {
      const r2 = await api.mapPolicies(sid);
    ...
    setStepMsg("Analysing compliance gaps...");
    try {
      const r3 = await api.analyzeGaps(sid);
      setGaps(r3.gaps);
    ...
    setStepMsg("Generating policy pull requests...");
    try {
      const r4 = await api.generatePRs(sid);
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: "var(--navy)" }}>
      ...
      <main className="max-w-7xl mx-auto px-6 py-8 w-full flex-1">
        ...
        {activeTab === "export" && (
          <ExportTab sid={sid} obligations={obligations} gaps={gaps} prs={prs} reviews={reviews} />
        )}
      </main>
      ...
      {reviewingPR && (
        <ReviewModal pr={reviewingPR}
          ...
          onSubmit={submitReview} onClose={() => setReviewingPR(null)} />
      )}
    </div>
  );
}
...
// ─── Upload Tab ────────────────────────────────────────────────────────────────
function UploadTab({ regulation, policies, matrix, sessionReady, uploading, isRunning, pipeline, onRegulation, onAddPolicy, onRemovePolicy, onMatrix, onCreateSession, onRunPipeline }: {
        ...
        <div className="card p-5" style={{ borderColor: regulation ? "rgba(16,185,129,0.3)" : "rgba(0,180,216,0.15)" }}>
          ...
          {regulation
            ? <div className="flex items-center gap-2 text-sm p-2 rounded" style={{ background: "rgba(16,185,129,0.05)", border: "1px solid rgba(16,185,129,0.1)" }}>
                ...
                <button onClick={() => onRegulation(null as unknown as File)} className="ml-auto"><X size={12} style={{ color: "var(--silver)" }} /></button>
              </div>
        ...
        <div className="card p-5" style={{ borderColor: matrix ? "rgba(16,185,129,0.3)" : "rgba(0,180,216,0.15)" }}>
          ...
          {matrix
            ? <div className="flex items-center gap-2 text-sm p-2 rounded" style={{ background: "rgba(16,185,129,0.05)", border: "1px solid rgba(16,185,129,0.1)" }}>
                ...
                <button onClick={() => onMatrix(null as unknown as File)} className="ml-auto"><X size={12} style={{ color: "var(--silver)" }} /></button>
              </div>
            : <label className="flex items-center gap-2 text-sm px-3 py-2 rounded-lg w-fit cursor-pointer" style={{ border: "1px dashed rgba(0,180,216,0.3)", color: "var(--teal)" }}>
...
// ─── Obligations Tab ───────────────────────────────────────────────────────────
function ObligationsTab({ obligations, mapMap, gapMap, prMap, reviewMap, expandedObl, onToggle }: {
  ...
  expandedObl: string | null; onToggle: (id: string) => void;
}) {
          ...
          <table className="w-full text-sm">
            <thead>
              <tr style={{ borderBottom: "1px solid rgba(0,180,216,0.1)" }}>
                {["ID", "Obligation", "Domain", "Coverage", "Risk", "Gap Explanation", "Action"].map(h => (
...
function AuditTab({ trail, events }: { trail: AuditTrailEntry[]; events: { event_type: string; obligation_id: string; actor: string; data: Record<string, unknown>; timestamp: string }[] }) {
  ...
  return (
    <div>
      ...
      {trail.length > 0 && (
        <div className="mb-8">
          <div className="text-xs font-semibold mb-3 uppercase tracking-wider" style={{ color: "var(--silver)" }}>Obligation Traceability ({trail.length})</div>
          <div className="grid gap-2">
            {trail.map((t, i) => (
              <div key={t.obligation_id} className="card p-4 text-xs">
                <div className="grid grid-cols-5 gap-3">
                  ...
                  <div>
                    ...
                    {t.gap_analysis ? <><span className={`badge-${t.gap_analysis.coverage === "Fully Covered" ? "fully" : t.gap_analysis.coverage === "Partially Covered" ? "partial" : "not"}`}>{t.gap_analysis.coverage}</span><div className="mt-1" style={{ color: "var(--silver)" }}>{t.proposed_amendment?.owner || "—"}</div></> : <div style={{ color: "rgba(148,163,184,0.3)" }}>—</div>}
                  </div>
                  <div>
                    <div className="font-semibold mb-1" style={{ color: "var(--silver)" }}>DECISION</div>
                    {t.review_decision ? <><span className={`badge-${t.review_decision.action.toLowerCase()}`}>{t.review_decision.action}</span><div className="mt-1" style={{ color: "rgba(148,163,184,0.4)" }}>{new Date(t.review_decision.timestamp).toLocaleDateString()}</div></> : <div style={{ color: "rgba(148,163,184,0.3)" }}>Pending</div>}
                  </div>
                </div>
              </div>
            ))}
...
// ─── Export Tab ────────────────────────────────────────────────────────────────
function ExportTab({ sid, obligations, gaps, prs, reviews }: { sid: string; obligations: Obligation[]; gaps: GapAnalysis[]; prs: PolicyPR[]; reviews: ReviewDecision[] }) {
  return (
      ...
      <div className="card p-5 mb-4">
        ...
        <div className="grid grid-cols-2 gap-2 text-sm">
          {[
            ...
            { label: "Policy Mappings",  value: gaps.length },
...
// ─── Review Modal ──────────────────────────────────────────────────────────────
function ReviewModal({ pr, obligation, notes, modifiedText, reviewer, existingReview, onNotesChange, onModifiedChange, onReviewerChange, onSubmit, onClose }: {
  ...
  onSubmit: (action: string) => void; onClose: () => void;
}) {
