...
export interface Session {
  session_id: string;
  regulation: { name: string; text_length: number; preview: string };
  policies: { name: string; text_length: number; preview: string }[];
...
export interface AuditTrailEntry {
  ...
  source_citation: string; domain: string;
...
export const api = {
  ...
  getSession: (sid: string) => req<{ session_id: string; regulation_name: string; policy_names: string[]; created_at: string }>(`/api/upload/session/${sid}`),
