...
Return a JSON array. Each element must have EXACTLY these keys:
{
  ...
  "full_text": "The full regulatory text this obligation comes from (1-3 sentences)",
  "source": "Section/Article reference e.g. 'ICT Risk Governance, para 1' or 'Third-Party ICT Providers, bullet 2'",
...
Use IDs OBL-001, OBL-002, etc."""
    try:
        return _parse(_call(prompt, max_tokens=4096))
    except Exception:
...
MAPPING_SYSTEM = """You are a compliance policy analyst. Your task is to map each regulatory obligation to the most relevant section of the internal policy documents provided.
...
async def map_policies(obligations: list, policy_texts: dict) -> list:
    policy_block = "\n\n".join(
        f"=== POLICY: {name} ===\n{text}" for name, text in policy_texts.items()
    )
    obls_json = json.dumps(
        [{"id": o["id"], "statement": o["statement"], "domain": o["domain"]} for o in obligations],
        indent=2,
    )
...
- Use the owner from the responsibility matrix where the domain matches"""
    try:
        return _parse(_call(prompt, max_tokens=6000))
    except Exception:
...
def _fallback_obligations():
    return [
        {"id":"OBL-001","statement":"Maintain documented ICT risk management procedures","full_text":"Financial entities shall maintain documented ICT risk management procedures covering the identification of critical systems, ownership, reviews and escalation.","source":"ICT Risk Governance, para 1","domain":"ICT Risk","confidence":0.97,"notes":"Covers entire ICT risk governance framework"},
        {"id":"OBL-002","statement":"Identify critical ICT systems within the risk management framework","full_text":"The procedures shall identify critical ICT systems.","source":"ICT Risk Governance, para 1, bullet 1","domain":"ICT Risk","confidence":0.96,"notes":""},
        {"id":"OBL-003","statement":"Define ownership responsibilities for ICT risk procedures","full_text":"The procedures shall define ownership responsibilities.","source":"ICT Risk Governance, para 1, bullet 2","domain":"Governance","confidence":0.95,"notes":""},
        {"id":"OBL-004","statement":"Conduct annual ICT risk reviews","full_text":"The procedures shall establish annual risk reviews.","source":"ICT Risk Governance, para 1, bullet 3","domain":"ICT Risk","confidence":0.97,"notes":"Annual frequency is mandatory — 'periodically' is insufficient"},
        {"id":"OBL-005","statement":"Maintain evidence of ICT risk assessments","full_text":"The procedures shall maintain evidence of risk assessments.","source":"ICT Risk Governance, para 1, bullet 4","domain":"ICT Risk","confidence":0.96,"notes":"Evidence must be retained and producible"},
        {"id":"OBL-006","statement":"Document incident escalation procedures","full_text":"The procedures shall document incident escalation procedures.","source":"ICT Risk Governance, para 1, bullet 5","domain":"Incident Management","confidence":0.95,"notes":""},
        {"id":"OBL-007","statement":"Review critical ICT vendors at least annually","full_text":"Financial entities shall review critical ICT vendors at least annually.","source":"Third-Party ICT Providers, bullet 1","domain":"Vendor Management","confidence":0.98,"notes":"'Periodically' does not satisfy this — annual frequency required"},
        {"id":"OBL-008","statement":"Maintain evidence of vendor assessments","full_text":"Financial entities shall maintain evidence of vendor assessments.","source":"Third-Party ICT Providers, bullet 2","domain":"Vendor Management","confidence":0.96,"notes":""},
        {"id":"OBL-009","statement":"Define responsible owners for vendor oversight","full_text":"Financial entities shall define responsible owners for vendor oversight.","source":"Third-Party ICT Providers, bullet 3","domain":"Vendor Management","confidence":0.95,"notes":""},
        {"id":"OBL-010","statement":"Establish escalation procedures for unresolved vendor risks","full_text":"Financial entities shall establish escalation procedures for unresolved vendor risks.","source":"Third-Party ICT Providers, bullet 4","domain":"Vendor Management","confidence":0.94,"notes":""},
        {"id":"OBL-011","statement":"Maintain documented incident response procedures","full_text":"Financial entities shall maintain documented incident response procedures.","source":"Incident Management, bullet 1","domain":"Incident Management","confidence":0.97,"notes":""},
        {"id":"OBL-012","statement":"Conduct annual incident-response testing","full_text":"Financial entities shall conduct annual incident-response testing.","source":"Incident Management, bullet 2","domain":"Incident Management","confidence":0.98,"notes":"Annual testing is mandatory"},
        {"id":"OBL-013","statement":"Preserve evidence of incident-response testing activities","full_text":"Financial entities shall preserve evidence of testing activities.","source":"Incident Management, bullet 3","domain":"Incident Management","confidence":0.96,"notes":""},
        {"id":"OBL-014","statement":"Document post-incident reviews after every significant incident","full_text":"Financial entities shall document post-incident reviews.","source":"Incident Management, bullet 4","domain":"Incident Management","confidence":0.95,"notes":"'May be documented' is insufficient — mandatory documentation required"},
    ]
...
def _fallback_gaps(obligations):
    specific = {
        ...
        "OBL-011":("Partially Covered","Medium","The policy states the organisation 'shall maintain an incident response process' but does not require it to be formally documented with defined steps, roles, and timelines.","Incident Management, bullet 1","Require formal documented procedure with defined steps, roles, timescales, and review cycle."),
        "OBL-012":("Not Covered","High","The Incident Response Policy contains no requirement for annual testing. This obligation is entirely absent.","Incident Management, bullet 2","Add mandatory annual incident-response testing with documented test plan and results."),
        "OBL-013":("Not Covered","High","No requirement to preserve evidence of testing activities. No documentation or retention obligation for test results.","Incident Management, bullet 3","Add requirement to document and retain evidence of all tests for minimum 3 years."),
        "OBL-014":("Not Covered","High","Policy states lessons learned 'may be documented after major incidents' — discretionary language. 'May' allows non-compliance.","Incident Management, bullet 4","Replace 'may be documented' with 'shall be documented' and define mandatory post-incident review process."),
    }
    gaps = []
    for i, ob in enumerate(obligations):
